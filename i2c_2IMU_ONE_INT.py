...
bus.write_byte_data(IMU1_addr,CTRL1,ODR_XL2)

# one shot write to INT1 on 2nd IMU 
# 2nd IMU Accelerometor data on INT1
#bus.write_byte_data(IMU2_addr,INT1,INT1_DRDY_XL)
...
bus.write_byte_data(IMU1_addr,FIFO_CTRL3,BDR_XL2)
# set FIFO in FIFO mode.
bus.write_byte_data(IMU1_addr,FIFO_CTRL4,FIFO_MODE0)
...
bus.write_byte_data(IMU1_addr,INT1,INT2_FIFO_FULL)
...
def my_callback(channel): 
	...
	def check():
		...
		status3 = bus.read_byte_data(IMU1_addr,FIFO_STATUS1)
		print(status3)
		status1 = bus.read_byte_data(IMU1_addr,FIFO_STATUS2)
		print(status1)
		status4 = bus.read_byte_data(IMU2_addr,FIFO_STATUS1)
		print(status4)
		status2 = bus.read_byte_data(IMU2_addr,FIFO_STATUS2)
		print(status2)
		if (status1 & FIFO_FULL_IA):
			...
			while (count <=500 ) :
				...
				IMU = bus.read_i2c_block_data(IMU2_addr,FIFO_DATA_OUT_TAG,7)
				IMU = bus.read_i2c_block_data(IMU1_addr,FIFO_DATA_OUT_TAG,7)
				print(IMU)
				count += 1
			count = 0
			print('read 1024 1st IMU data')
	if GPIO.input(24):
		print('pin high')
